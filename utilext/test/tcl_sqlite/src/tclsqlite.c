 ...
 * 2021 January 16
 *
 * Modified to include the 'limit' command to a Tcl sqlite database object,
 * which invokes the "sqlite3_limit()" API.
 *
...
deserialize_error:
    ...
    **
    ** Set or query runtime limits using the sqlite3_db_limit()
    ** interface.
    */
    case DB_LIMIT: {
      static const struct {
      ...
      if (objc > 4) {
        Tcl_WrongNumArgs(interp, 2, objv, "?OPTION? ?INTEGER?");
        return TCL_ERROR;
      }
      if (objc == 2) {
        /* With no arguments, list all limit options and with the
        ** current value */
        pResult = Tcl_NewListObj(0, 0);
        for (ii = 0; ii < sizeof(aDbLimit) / sizeof(aDbLimit[0]); ii++) {
          v = sqlite3_limit(pDb->db, aDbLimit[ii].op, val);
          Tcl_ListObjAppendElement(interp, pResult,
                                   Tcl_NewStringObj(aDbLimit[ii].zName, -1));
          Tcl_ListObjAppendElement(interp, pResult,
                                   Tcl_NewIntObj(v));
        }
      }
        ...
        if (zOpt[0] == '-') zOpt++;
        for (ii = 0; ii < sizeof(aDbLimit) / sizeof(aDbLimit[0]); ii++) {
          if (strcmp(aDbLimit[ii].zName, zOpt) == 0) break;
        }
        if (ii >= sizeof(aDbLimit) / sizeof(aDbLimit[0])) {
          Tcl_AppendResult(interp, "unknown limit option: \"", zOpt,
                           "\"", (void*)0);
          return TCL_ERROR;
        }
        if (objc == 4) {
